...
def randomized_benchmarking_seq(nseeds=1, length_vector=None,
    ...
    if rb_pattern is None:
        rb_pattern = [[0]]
    rb_qubits = []
    for quset in rb_pattern: 
        for qubitnum in quset:
            rb_qubits.add(qubitnum)
    ...
    if is_leakage:
        num_meas = 2^(n_q_max+1)
    else:
        num_meas=1

    for meas_basis in range(num_meas):
            ...
            for cliff_index in range(length_vector[-1]):
                ...
                if (cliff_index+1) == length_vector[length_index]:
                    ...
                    for qind, qb in enumerate(qlist_flat)
                        ...
                        for meas_x in n_q_max+1:
                            if bin(num_meas)[meas_x] == '1':
                                circ.x[qr[qb][meas_x]]
        ...
        circuits_list.append([circuits, xdata])
    if is_leakage: 
        return ciruits_list
    else: 
